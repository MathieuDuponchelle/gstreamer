 ...
 *   their template caps. This is much faster than _GST_PAD_LINK_CHECK_CAPS_, but
 ...
 *
 * The amount of checking to be done when linking pads. _GST_PAD_LINK_CHECK_CAPS_
 * and _GST_PAD_LINK_CHECK_TEMPLATE_CAPS_ are mutually exclusive. If both are
 * specified, expensive but safe _GST_PAD_LINK_CHECK_CAPS_ are performed.
 *
 * <warning><para>
 ...
 * @pad: a [GstPad]()
 * @parent: the parent of _pad_
 * @mode: the requested activation mode of _pad_
 ...
 * @parent: (allow-none): the parent of _pad_. If the [GST_PAD_FLAG_NEED_PARENT]()
 *          flag is set, _parent_ is guaranteed to be not-[NULL]() and remain valid
 ...
 *
 * The implementer of this function receives a refcount to _buffer_ and should
 ...
 * either consumes the data or passes it on to the internally linked pad(s).
 *
 * The implementer of this function receives a refcount to _list_ and
 ...
 * request a buffer at the specified _offset_ and _length_. If this function
 * returns [GST_FLOW_OK](), the result buffer will be stored in _buffer_. The
 * contents of _buffer_ is invalid for any other return value.
 *
 ...
 * [GST_PAD_MODE_PULL]().
 *
 * _offset_ and _length_ are always given in byte units. _offset_ must normally be a value
 * between 0 and the length in bytes of the data available on _pad_. The
 * length (duration in bytes) can be retrieved with a [GST_QUERY_DURATION]() or with a
 * [GST_QUERY_SEEKING]().
 *
 * Any _offset_ larger or equal than the length will make the function return
 * [GST_FLOW_EOS](), which corresponds to EOS. In this case _buffer_ does not
 * contain a valid buffer.
 *
 * The buffer size of _buffer_ will only be smaller than _length_ when _offset_ is
 * near the end of the stream. In all other cases, the size of _buffer_ must be
 * exactly the requested size.
 *
 * It is allowed to call this function with a 0 _length_ and valid _offset_, in
 * which case _buffer_ will contain a 0-sized buffer and the function returns
 * [GST_FLOW_OK]().
 *
 * When this function is called with a -1 _offset_, the sequentially next buffer
 * of length _length_ in the stream is returned.
 *
 * When this function is called with a -1 _length_, a buffer with a default
 * optimal length is returned in _buffer_. The length might depend on the value
 * of _offset_.
 *
 * Returns: [GST_FLOW_OK]() for success and a valid buffer in _buffer_. Any other
 * return value leaves _buffer_ undefined.
 */
 ...
 *     @_GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM_ or
 *     @_GST_PAD_PROBE_TYPE_EVENT_UPSTREAM_ probe types.
 ...
 * _GST_PAD_PROBE_TYPE_IDLE_ or _GST_PAD_PROBE_TYPE_BLOCK_ is used, the probe will be a
 * blocking probe.
 */
typedef enum
{
 ...
 * @data: (allow-none): type specific data, check the _type_ field to know the
 *    datatype.  This field can be [NULL]().
 * @offset: offset of pull probe, this field is valid when _type_ contains
 *    [GST_PAD_PROBE_TYPE_PULL]()
 * @size: size of pull probe, this field is valid when _type_ contains
 *    [GST_PAD_PROBE_TYPE_PULL]()
 *
 ...
 * blocking type.
 *
 * The callback is allowed to modify the data pointer in _info_.
 *
 ...
 *
 * When _event_ is set to [NULL](), the item will be removed from the list of sticky events.
 * _event_ can be replaced by assigning a new reference to it.
 ...
 * @pad: a [GstPad]()
 *
 * Get the private data of _pad_, which is usually some pad- or stream-specific
 ...
 * @pad: a [GstPad]()
 *
 * Get the _pad_ [GstPadTemplate](). It describes the possible media types
 * a _pad_ or an element factory can handle.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadDirection]() of the given _pad_. Accessor macro, use
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstTask]() of _pad_. Accessor macro used by GStreamer. Use the
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadActivateFunction]() from _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadActivateModeFunction]() from the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadChainFunction]() from the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadChainListFunction]() from the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadGetRangeFunction]() from the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadEventFunction]() from the given _pad_, which
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadQueryFunction]() from _pad_, which is the function
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadIterIntLinkFunction]() from the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Return the pad's peer member. This member is a pointer to the linked _pad_.
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadLinkFunction]() for the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the [GstPadUnlinkFunction]() from the given _pad_.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Check if the dataflow on a _pad_ is blocked. Use [gst_pad_is_blocked]() instead.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Check if the _pad_ is currently blocking on a buffer or event. Use
 ...
 * @pad: a [GstPad]()
 *
 * Check if the given _pad_ is flushing.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Set the given _pad_ to flushing state, which means it will not accept any
 ...
 * @pad: a [GstPad]()
 *
 * Check if the _pad_ is in EOS state.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Check if the _pad_ should be reconfigured/renegotiated.
 ...
 * @pad: a [GstPad]()
 *
 * Check if the given _pad_ has pending events. This is used internally by
 * GStreamer.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Check if the given _pad_ is using fixed caps, which means that
 * once the caps are set on the _pad_, the caps query function will
 ...
 * @pad: a [GstPad]()
 *
 * Check if there is a parent object before calling into the _pad_ callbacks.
 ...
 * @pad: a [GstPad]()
 *
 * Check if the given _pad_ is set to proxy caps. This means that the default
 * event and query handler will forward all events and queries to the
 * internally linked _pads_ instead of discarding them.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Set _pad_ to proxy caps, so that all caps-related events and queries are
 ...
 * @pad: a [GstPad]()
 *
 * Check if the given _pad_ is set as proxy allocation which means
 ...
 * @pad: a [GstPad]()
 *
 * Set _pad_ to proxy allocation queries, which means that the default query
 * handler will forward allocation queries to the internally linked _pads_
 * instead of discarding them.
 * Set this if the element always outputs data in the exact same format as it
 * receives as input. This is just for convenience to avoid implementing some
 ...
 * @pad: a [GstPad]()
 *
 * Check if the given _pad_ is set to proxy scheduling queries, which means that
 * the default query handler will forward scheduling queries to the internally
 * linked _pads_ instead of discarding them.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Set _pad_ to proxy scheduling queries, which means that the default query
 * handler will forward scheduling queries to the internally linked _pads_
 ...
 * @pad: a [GstPad]()
 *
 * Set _pad_ to by default accept caps by intersecting the result instead of
 * checking for a subset. This is interesting for parser elements that can
 * accept incompletely specified caps.
 */
 ...
 * @pad: a [GstPad]()
 *
 * Get the stream lock of _pad_. The stream lock is protecting the
 * resources used in the data processing functions of _pad_. Accessor
 ...
 *
 * Get the parent of _pad_. This function increases the refcount
